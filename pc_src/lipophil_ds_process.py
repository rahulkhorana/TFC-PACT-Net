...
def load_lipophil_data():
    ...
    df.rename(columns={"exp": "y"}, inplace=True)
    return df[["smiles", "y"]]


# -- Graph Construction --
def build_graph_from_smiles(smile: str, topk_lap: int = 5) -> Data | None:
    try:
        # 1) Abstract complex
...
def process_smiles(smile, y):
    g = build_graph_from_smiles(smile)
    if g is not None:
        g.y = torch.tensor([y], dtype=torch.float32)
    return g


def load_dataset_parallel(n_jobs=None):
    df = load_lipophil_data()
    if n_jobs is None:
        n_jobs = max(1, multiprocessing.cpu_count() - 2)
    results = Parallel(n_jobs=n_jobs)(
        delayed(process_smiles)(row.smiles, row.y) for _, row in df.iterrows()
    )
    return [g for g in results if g is not None]


if __name__ == "__main__":
    all_data = load_dataset_parallel()
    print(f"Built {len(all_data)} graphs")
    torch.save(all_data, "richer_lipophil_data.pt")
